...
Exposes PocketPortal tools as an MCP server, allowing other
applications to connect to PocketPortal and use its tools.

This enables:
- Other Claude Desktop instances to use PocketPortal tools
- Third-party applications to integrate with PocketPortal
- Bidirectional communication (PocketPortal ↔ Other apps)

Example:
--------
User's Claude Desktop can use PocketPortal's tools while
PocketPortal can use Claude Desktop's tools - full mesh.
"""
...
class MCPServer:
    """
    MCP Server implementation for PocketPortal.

    Exposes PocketPortal's tools via the Model Context Protocol,
    ...
    def __init__(self, tool_registry: Optional[Any] = None):
        ...
        self.server = Server("pocketportal")
    ...
    def _setup_handlers(self):
        ...
        async def list_tools() -> List[types.Tool]:
            ...
            # Convert PocketPortal tools to MCP tool format
            tools = []
...
async def start_mcp_server(tool_registry: Optional[Any] = None):
    ...
    This allows PocketPortal to be used as an MCP server
...
def register_mcp_server_command(cli_parser):
    ...
    Usage:
        pocketportal mcp-server
    """
    parser = cli_parser.add_parser(
        'mcp-server',
        help='Start PocketPortal as an MCP server'
    )

    def handle_mcp_server(args):
        """Handle mcp-server command"""
        print("Starting PocketPortal MCP Server...")
        print("Expose PocketPortal tools to other applications via MCP.")
        print()

        # Import here to avoid circular imports
        from pocketportal.tools import ToolRegistry
